...
function UserCard({ user, onUserDeleted }) {
  ...
  return (
    <div className="bg-white rounded-lg shadow-lg border-gray-800 mb-3 hover:bg-gray-100 hover:cursor-pointer w-full mt-9">
      ...
      <div className="p-4">
        <h2 className="text-xl text-slate-600 font-bold my-2">{user.nombre} {user.apellido_paterno} {user.apellido_materno}</h2>
        <p>{user.email}</p>
        <div className="w-full h-[45px] flex justify-start items-center mt-3">
          <button
            className="w-16 h-10 text-white bg-blue-900 bg-opacity-50 flex items-center justify-center rounded-lg shadow-md hover:bg-opacity-80"
            onClick={handleDelete}
          >
            <Trash size={20} />
          </button>
          <button
            className="w-16 h-10 text-white bg-blue-900 bg-opacity-50 flex items-center justify-center rounded-lg shadow-md hover:bg-opacity-80 ml-3"
            onClick={handleEdit}
          >
            <Edit size={20} />
          </button>
